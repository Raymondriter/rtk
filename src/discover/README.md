...
**Compound splitting** — The rewrite engine walks the tokens, splitting on `Operator` (`&&`, `||`, `;`) and `Pipe` (`|`, `|&`). Each segment is rewritten independently. A stage feeding a pipe is rewritten only when every downstream stage is display-only (`head`, `tail` without a follow flag, `cat`): those stages just bound what the agent reads, so rtk-shaped output is still fine. Any content-consuming stage (`wc`, `xargs`, …) keeps the producer raw — rtk's reshaped output corrupts programs that parse it (#2962, #1560, #439). The **final** stage of a pipe group is the exception: its stdout goes to the agent, so it is rewritten when its resolved command is in `STDIN_SAFE_FINAL` (`grep`, `rg` — filters that read stdin, stream instead of buffering to EOF, and preserve exit codes). Stage identity is resolved through the same wrapper stripping as the rewriter itself (sudo/env, shell builtins, transparent_prefixes, absolute paths), so `command wc -l` cannot bypass the check and `command grep` still qualifies. Intermediate stages are never rewritten.
